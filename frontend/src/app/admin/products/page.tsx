...
export default function AdminProductsPage() {
  ...
  const [formData, setFormData] = useState<ProductFormData>({
    ...
    imageUrl: '',
    isActive: true,
    listings: [{ size: '', price: '', stock: '0', skuCode: '', isActive: true }],
  });
  ...
  const addListing = () => {
    setFormData((prev) => ({
      ...
      listings: [
        ...prev.listings,
        { size: '', price: '', stock: '0', skuCode: '', isActive: true },
      ],
    }));
  };
  ...
  const removeListing = (index: number) => {
    setFormData((prev) => ({
      ...prev,
      listings: prev.listings.filter((_, i) => i !== index),
    }));
  };

  const updateListing = (index: number, field: keyof ListingInput, value: string | boolean) => {
    setFormData((prev) => ({
      ...prev,
      listings: prev.listings.map((listing, i) =>
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      // Build data matching backend API structure
      ...
      if (formData.imageUrl) {
        data.images = [{ imageUrl: formData.imageUrl, sortOrder: 0 }];
      }

      // Add listings
      data.listings = formData.listings
        ...
        .map((listing) => ({
          ...
          skuCode: listing.skuCode || undefined,
  ...
  const openEditModal = (product: Product) => {
    ...
    setFormData({
      ...
      categoryIds: product.categories?.map((c) => c.categoryId?.toString()) || [],
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {products.map((product) => (
                <tr key={product.id}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm text-gray-900">
                    {product.listings?.reduce((sum, item) => sum + (item.stock || 0), 0) ||
                      product.stock ||
                      product.items?.reduce((sum, item) => sum + item.stock, 0) ||
                      0}
                  </td>
                  ...
                  <div className="col-span-2">
                    <div className="flex items-center justify-between mb-2">
                      <label className="block text-sm font-medium text-gray-700">
                        Product Variants (Listings) *
                      </label>
                      <Button type="button" variant="secondary" onClick={addListing}>
                        ...
                        >
                          <div className="col-span-2">
                            <input
                              ...
                              value={listing.size}
                              onChange={(e) =>
                                updateListing(index, 'size', e.target.value)
                              }
                          ...
                          <div className="col-span-2">
                            <input
                              ...
                              value={listing.price}
                              onChange={(e) =>
                                updateListing(index, 'price', e.target.value)
                              }
                              className="block w-full rounded-md border-gray-300 text-sm"
                              required={index === 0}
                            />
                          ...
                          <div className="col-span-2">
                            <input
                              ...
                              value={listing.stock}
                              onChange={(e) =>
                                updateListing(index, 'stock', e.target.value)
                              }
                              className="block w-full rounded-md border-gray-300 text-sm"
                            />
                          </div>
                          <div className="col-span-3">
                            <input
                              ...
                              value={listing.skuCode}
                              onChange={(e) =>
                                updateListing(index, 'skuCode', e.target.value)
                              }
                              className="block w-full rounded-md border-gray-300 text-sm"
                            />
                          </div>
                          <div className="col-span-2">
                            <label className="flex items-center text-sm">
                              <input
                                ...
                                onChange={(e) =>
                                  updateListing(index, 'isActive', e.target.checked)
                                }
